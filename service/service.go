...
import (
	...
	cloud_metadata "github.com/khulnasoft-lab/cloud-scanner/cloud-metadata"
	"github.com/khulnasoft-lab/cloud-scanner/cloud_resource_changes"
	"github.com/khulnasoft-lab/cloud-scanner/internal/khulnasoft"
	"github.com/khulnasoft-lab/cloud-scanner/query_resource"
	"github.com/khulnasoft-lab/cloud-scanner/scanner"
...
func NewComplianceScanService(config util.Config, socketPath *string) (*ComplianceScanService, error) {
	...
	dfClient, err := khulnasoft.NewClient(config)
	if err != nil {
		log.Error().Msgf("khulnasoft.NewClient(config) error: %s", err.Error())
		return nil, err
	}
	if config.CloudMetadata.ID == "" {
		log.Error().Msgf("empty cloud metadata id from khulnasoft.NewClient(config)")
